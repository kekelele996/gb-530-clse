...
@Component({
  ...
  imports: [
    ...
    MatSelectModule, BudgetEvidencePanelComponent, AssessmentFreshnessComponent,
      ...
      <div class="split">
        ...
        <div *ngIf="budget.selected() as selected" class="detail">
          ...
          <div class="review-row">
            ...
            <button *ngIf="auth.canPlan() && selected.assessment_status === 'submitted' && isStale(selected)" mat-flat-button color="primary"
                    [disabled]="running()" (click)="reassess(selected)" title="Supersede the stale snapshot with current records and limits">
              {{ running() ? 'Reassessing' : 'Reassess with current inputs' }}
            </button>
            <span *ngIf="selected.assessment_status === 'submitted' && !isStale(selected)" class="waiting">Awaiting independent RPO review</span>
            <span *ngIf="selected.assessment_status === 'superseded'" class="superseded">Superseded snapshot · retained for traceability</span>
          </div>
        </div>
      </div>
    </div>
  ...
  styles: [`
    ...
    .superseded { color: #34413e !important; font-weight: 700; }
...
export class BudgetsPage implements OnInit {
  ...
  isStale(assessment: DoseBudgetAssessment): boolean {
    return assessment.freshness?.fresh === false;
  }

  reassess(assessment: DoseBudgetAssessment): void {
    this.running.set(true); this.error.set('');
    this.budget.reassess(assessment.id, inputToUTC(this.assessmentForm.controls.period_end.value), assessment.plan_version)
      .pipe(finalize(() => this.running.set(false))).subscribe({
        next: () => this.plans.load(),
        error: error => this.error.set(apiErrorMessage(error)),
      });
  }
